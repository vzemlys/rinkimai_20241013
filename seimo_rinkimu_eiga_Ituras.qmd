...
### Ar LŽVS nukris žemiau 7?

```{r}
fc7 <- dt4 %>% filter(saraso_numeris == 8)

mod <- lm(proc_nuo_dal_rinkeju~lt_proc_rinkeju, data =fc7)

summary(mod)

predict(mod, newdata = data.frame(lt_proc_rinkeju = 100))

```

### Ar LP pakils aukščiau 5%?

```{r}
fc7 <- dt1 %>% filter(saraso_numeris == 16)
